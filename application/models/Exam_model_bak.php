...
public function GetCollege($yb_userid=7041045)
{
  ...
  public function UpdateCollege($college='物电�?)
  {
    $this->db->select('college');
...
else
{
  ...
  public function GetScore($id,$anwser)
  {
    ...
    // echo '单�?.$xu_id.'成绩'.$score.'总数'.$this->RadioNum()['RadioNum'].'<br \>';
    foreach ($InputId['MultiId'] as $value)
    {
      ...
      if($query->num_rows() > 0)  //答案正确
      {
          ...
          //ChromePhp::log($value."多选答案正�?".$score."<br \>");
      }
      else        //答案多�?      {
        ...
        if(strlen($query->row_array()['anwser']) < strlen(implode('',$anwser[$num])))
        {
          ...
          //ChromePhp::log($value."�?.$score."<br \>");
        }
        else        //答案少�?        {
          $cuoxuan = 0;   //错选标�?          foreach ($_anwser[$value] as $key)
          {
            $this->db->select('anwser');
            $query = $this->db->get_where('Multiple',array('id'=>$value));
            if(strpos($query->row_array()['anwser'],$key)===FALSE)  //答案错�?            {
              $cuoxuan = 1;
              //ChromePhp::log($value."错�?");
            }
            // echo $cuoxuan."<br \>";
          }
          if(!$cuoxuan)
