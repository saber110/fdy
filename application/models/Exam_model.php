...
require_once FCPATH.'debug/chromephp/ChromePhp.php';
...
class Exam_model extends CI_Model
{
    ...
    /**
     * @function 获取初复赛的各类题目数量
     */
    public function GetNum($value = '')
    {
      $this->db->select('topic,RadioNum,MultiNum,TorFNum');
      $query = $this->db->get('kaoshi');
      return $query->result_array();
    }
    public function GetCollege($yb_userid = 7041045)
    {
    ...
    public function UpdateExamination(
        ...
        $query = $this->db->get_where('examination', array('yb_userid' => $yb_userid));
    //var_dump($query->row_array()['most_score']);
        if ($query->row_array()['most_score'] == null) {
            $data = array(
        'yb_userid' => $yb_userid,
        ...
        'sparetime' => $time + $query->row_array()['sparetime'],
            ...
            return $this->db->insert('examination', $data);
        } else {
    ...
    public function PanduanNum($topic = '测试')
    {
    ...
    public function PanduanSum()
    {
      return $this->db->count_all('panduan');
    }
    ...
    else
    {
      ...
      foreach ($para['TorF'] as $id) {
          $this->db->select('id,topic');
          $query = $this->db->get_where('panduan', array('id' => $id));
          $TorFRes[$TorFNum++] = $query->result_array();
      }
      return array(
          'radio' => $RadioRes,
  ...
  public function RScore($id, $anwser,$type)
  {
    if($type != "复赛")
    {
      $num = 0;
      ...
      foreach ($id as $value)
      {
          if ($num < $this->RadioNum($type)['RadioNum']) {
              $RadioId[$radio_num++] = $value;
          } elseif ($num < ($this->RadioNum($type)['RadioNum'] + $this->MultiNum($type)['MultiNum'])) {
    ...
    else
    {
      ...
      foreach ($id as $value)
      {
          ...
          elseif($num < ($this->RadioNum($type)['RadioNum'] + $this->MultiNum($type)['MultiNum'] + $this->PanduanNum($type)['TorFNum'] ))
          {
              $TorFId[$TorFnum++]=$value;
          }
          ++$num;
...
return array(
  ...
  public function GetScore($id, $anwser,$type)
  {
    $InputId = $this->RScore($id, $anwser,$type);
    ...
    {
      $score = 0;
      $num = $this->RadioNum($type)['RadioNum'];   //标志答案次序
      foreach ($InputId['MultiId'] as $value) {
        // var_dump($InputId['MultiId']);var_dump($anwser[$num]);exit;
          $this->db->select('fenzhi');
          $query = $this->db->get_where('Multiple', array('id' => $value, 'anwser' => implode('', $anwser[$num])));
          if ($query->num_rows() > 0) {  //答案正确
            $score = $score + $this->GetFenzhi($value, 'Multiple')['fenzhi'];
      ...
      foreach ($InputId['TorFId'] as $key => $value)
      {
        $this->db->select('fenzhi');
        $query = $this->db->get_where('panduan', array('id' => $value, 'anwser' => $anwser[$num]));
        if ($query->num_rows() > 0) {
            $score = $score + $this->GetFenzhi($value, 'panduan')['fenzhi'];
        }
        ++$num;
      }
